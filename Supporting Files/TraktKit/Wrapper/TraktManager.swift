...
public class TraktManager {
    ...
    public func signOutAndRevokeAccessToken(completionHandler: SuccessCompletionHandler? = nil) throws {
        guard let accessToken = accessToken,
            let clientID = clientID,
            let clientSecret = clientSecret else {
                print("In order to revoke an access token, the user must first be authenticated")
                completionHandler?(.fail)
                return
        }
        
        let urlString = "https://api.trakt.tv/oauth/revoke"
        let url = URL(string: urlString)
        guard var request = mutableRequestForURL(url, authorization: false, HTTPMethod: .POST) else { completionHandler?(.fail); return }
        ...
        session.dataTask(with: request) { [weak self] (data, response, error) -> Void in
            guard
                let welf = self else { return }
            guard error == nil else { completionHandler?(.fail); return }
            ...
            welf.accessToken = nil
            welf.refreshToken = nil
