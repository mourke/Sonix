...
extension TraktManager {
    ...
    public typealias AnticipatedCompletionHandler = ObjectsCompletionHandler<TraktAnticipated>
    ...
    // MARK: Lists
    public typealias PopularListsCompletionHandler = PaginatedCompletionHandler<TraktPopularList>
    public typealias TrendingListsCompletionHandler = PaginatedCompletionHandler<TraktTrendingList>
    ...
    func performRequest(request: URLRequest, expectingStatusCode code: Int, completion: SuccessCompletionHandler?) -> URLSessionDataTask? {
        let datatask = session.dataTask(with: request) { (data, response, error) -> Void in
            guard error == nil else { completion?(.fail); return }
            
            // Check response
            guard
                let HTTPResponse = response as? HTTPURLResponse,
                HTTPResponse.statusCode == code
                else { completion?(.fail); return }
            
            // Check data
            guard data != nil else { completion?(.fail); return }
    ...
    func performRequest(request: URLRequest, expectingStatusCode code: Int, completion: @escaping checkinCompletionHandler) -> URLSessionDataTask? {
        let aCompletion: DataResultCompletionHandler = { (result: DataResultType) -> Void in
            ...
            case .success(let data):
                ...
                } else if let rawDict = try? JSONSerialization.jsonObject(with: data) as? [String : Any],
    ...
    func performRequest<T>(request: URLRequest, expectingStatusCode code: Int, completion: ((_ result: ObjectResultType<T>) -> Void)?) -> URLSessionDataTask? {
